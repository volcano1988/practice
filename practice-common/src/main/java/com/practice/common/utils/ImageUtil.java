...
//            return "R=" + this.R + " G=" + this.G + " B=" + this.B;
            return String.join(",", this.R + "", this.G + "", this.B + "");
        }
    }
    ...
    public List<Point> getThreeFamilly(List<PointGroup> rootPoint) {
        ...
        //去除与白色相近的颜色
        rootPoint.removeIf(i -> i.point.colorDistance(new Point(255, 255, 255)) < 30);
        //根据RGB的总和对颜色排序
        rootPoint.sort(new pointCampare());
        //如果颜色小于等于3个，直接输出
        if (rootPoint.size() <= 3) {
            for (PointGroup pointGroup : rootPoint) {
                pointList.add(pointGroup.point);
            }
            return pointList;
        }
     ...
     * @return
     */
    public List<String> getColorSolution(BufferedImage image, int rootPointNum) {
        this.image = image;
...
//      }
    ...
    public static void main(String[] args) {
        try {
            BufferedImage image = ImageIO.read(new URL("https://p2.img.cctvpic.com/oms/upload/image/20200509/CkcN4162DcmAeslnAATOzj-DZgI17.jpeg"));
            List<String> colorSolution = new ImageUtil().getColorSolution(image, 200);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
